    ...
    <>
      ...
      >
        ...
        <button
          ...
          onKeyDown={
            showDialog ? handleKeyDownHideDialog : handleKeyDownShowDialog
          }
        >
          {showDialog ? (
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          ) : (
        ...
        <nav className="hidden md:block">
          <ul className="h-16">
            {listItems.map((listItem) => (
              <li
                key={listItem}
                className="bg-gradient-to-r from-lime-green to-bright-cyan text-grayish-blue hover:text-dark-blue text-sm inline-block h-full"
              >
                <a
                  href="#"
                  className="bg-white flex justify-center items-center px-4 h-full hover:h-[95%]"
                >
                  {listItem}
                </a>
              </li>
      ...
      {showDialog && (
        <div
          className="bg-gradient-to-b from-gray-800 to-transparent absolute flex justify-center items-center w-full top-0 bottom-0 z-10"
          role="dialog"
        >
                ...
                <li key={listItem} className="py-2">
                  {listItem === "Home" ? (
                    <a
                      ...
                      onKeyDown={handleKeyDownFirstListItem}
                    >
                      {listItem}
                    </a>
                  ) : listItem === "Careers" ? (
                    <a
                      href="#"
                      ref={lastListItemRef}
                      onKeyDown={handleKeyDownLastListItem}
                    >
                      {listItem}
                    </a>
                  ) : (
                    <a href="#" onKeyDown={handleKeyDownListItem}>
                      {listItem}
                    </a>
                  )}
                </li>
